...
<div class="table-card">
    <div class="table-header">
        <div class="table-header-left">
            <form action="{{ route('events.index') }}" method="GET" class="per-page-form">
                @if(request('search'))
        ...
        <div class="table-header-right">
            <form action="{{ route('events.index') }}" method="GET" class="table-search-form">
                @if(request('per_page'))
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($events as $index => $event)
                    <tr>
                            <td class="td-number">{{ $events->firstItem() + $index }}</td>
                        <td>
                            <div class="user-info">
                        ...
                        <td>
                            <span class="date-cell">{{ $event->start_at->format('M d, Y') }}</span>
                        </td>
                        <td>
                            <span class="date-cell">{{ $event->end_at->format('M d, Y') }}</span>
                        </td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('events.show', $event) }}" class="btn-icon btn-icon-view" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                    </svg>
                                </a>
                                <a href="{{ route('events.edit', $event) }}" class="btn-icon btn-icon-edit" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                </a>
                                <form action="{{ route('events.destroy', $event) }}" method="POST" class="inline-form" data-confirm-delete="Are you sure you want to delete this event?" data-delete-title="Delete Event">
                                    @csrf
                                    @method('DELETE')
