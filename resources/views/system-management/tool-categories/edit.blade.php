...
@section('breadcrumb')
    ...
    <span class="breadcrumb-item active">Edit Tool Category</span>
@endsection

@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>Edit Tool Category</h2>
            <p>Update tool category information</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ route('system-management.tool-categories.update', $toolCategory) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="tool-category-name" name="name" class="form-input @error('name') error @enderror"
                        value="{{ old('name', $toolCategory->name) }}" placeholder="Enter name" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="category-slug" name="slug" class="form-input @error('slug') error @enderror"
                        value="{{ old('slug', $toolCategory->slug) }}" placeholder="Enter slug" required>
                    @error('slug')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
            ...
            <div class="form-grid">
                <div class="form-group full-width">
                    ...
                    <textarea id="description" name="description" class="form-input @error('description') error @enderror"
                         placeholder="Enter description">{{ old('description', $toolCategory->description) }}</textarea>
                    @error('description')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('system-management.tool-categories.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
@endsection
