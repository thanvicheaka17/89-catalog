...
@section('title', 'Create Top Tier Tool')
...
@section('breadcrumb')
    ...
    <span class="breadcrumb-item active">Create Top Tier Tool</span>
@endsection

@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>Create Top Tier Tool</h2>
            <p>Add a new top tier tool to the system</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ route('game-management.tools.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-grid form-grid-3">
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" id="tool-category-id" class="form-input @error('category_id') error @enderror" required>
                        <option value="">Select a category</option>
                        @foreach($toolCategories as $toolCategory)
                            <option value="{{ $toolCategory->id }}" {{ old('category_id') == $toolCategory->id ? 'selected' : '' }}>{{ $toolCategory->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <span class="form-error">{{ $message }}</span>
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <select name="tier" id="tier" class="form-input @error('tier') error @enderror" required>
                        ...
                        <option value="platinum" {{ old('tier') == 'platinum' ? 'selected' : '' }}>Platinum
                        </option>
                    </select>
                </div>
                <div class="form-group">
