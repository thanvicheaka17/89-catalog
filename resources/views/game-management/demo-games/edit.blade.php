...
@section('breadcrumb')
    ...
    <span class="breadcrumb-item active">Edit Demo Game</span>
@endsection

@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>Edit Demo Game</h2>
            <p>Edit the demo game</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ route('game-management.demo-games.update', $demoGame) }}" method="POST"
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="category-name" name="title"
                        class="form-input @error('title') error @enderror" value="{{ $demoGame->title }}"
                ...
                <div class="form-group">
                    ...
                    <select id="is_demo" name="is_demo" class="form-input @error('is_demo') error @enderror" required>
                        <option value="1" {{ $demoGame->is_demo == 1 ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ $demoGame->is_demo == 0 ? 'selected' : '' }}>No</option>
                    </select>
                    @error('is_demo')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="category-slug" name="slug"
                        class="form-input @error('slug') error @enderror" value="{{ old('slug', $demoGame->slug) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="url" id="url" name="url" class="form-input @error('url') error @enderror"
                        value="{{ old('url', $demoGame->url) }}" placeholder="Enter URL">
                    @error('url')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
            ...
            <div class="form-grid">
                <div class="form-group full-width">
                    ...
                    <div class="image-upload-wrapper">
                        <div class="image-drop-zone {{ $demoGame->hasImage() ? 'has-file' : '' }}" id="imageDropZone">
                            <input type="file" id="imageInput" name="image" class="image-file-input"
                                accept="image/jpeg,image/png,image/gif,image/webp">
                            <div class="drop-zone-content" style="{{ $demoGame->hasImage() ? 'display: none;' : '' }}">
                            ...
                            <div class="image-preview {{ $demoGame->hasImage() ? 'has-image' : '' }}" id="imagePreview">
                                @if ($demoGame->hasImage())
                                    <div class="image-preview-item">
                                        <img src="{{ $demoGame->getImageUrl() }}" alt="Current demo game image">
                            ...
                            <button type="button" class="remove-image-btn" id="removeImage"
                                style="{{ $demoGame->hasImage() ? 'display: flex;' : 'display: none;' }}"
                                title="Remove image">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <div class="form-grid">
                <div class="form-group full-width">
                    ...
                    <textarea id="description" name="description" class="form-input @error('description') error @enderror"
                        placeholder="Enter description">{{ old('description', $demoGame->description) }}</textarea>
                    @error('description')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
