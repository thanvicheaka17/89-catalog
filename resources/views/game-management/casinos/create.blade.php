...
@section('breadcrumb')
    ...
    <span class="breadcrumb-item active">Create Casino</span>
@endsection
...
@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>Create Casino</h2>
            <p>Add a new casino to the system</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ route('game-management.casinos.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" id="casino-category-id" class="form-input @error('category_id') error @enderror" required>
                        ...
                        @foreach($casinoCategories as $casinoCategory)
                            <option value="{{ $casinoCategory->id }}" {{ old('category_id') == $casinoCategory->id ? 'selected' : '' }}>{{ $casinoCategory->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <span class="form-error">{{ $message }}</span>
