...
<div class="page-header">
    ...
    <div class="page-header-right">
        <a href="{{ route('blog-posts.create') }}" class="btn btn-info">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
<div class="table-card">
    <div class="table-header">
        <div class="table-header-left">
            <form action="{{ route('blog-posts.index') }}" method="GET" class="per-page-form">
                @if(request('search'))
        ...
        <div class="table-header-right">
            <form action="{{ route('blog-posts.index') }}" method="GET" class="table-search-form">
                @if(request('per_page'))
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($blogPosts as $index => $post)
                    <tr>
                        <td class="td-number">{{ $blogPosts->firstItem() + $index }}</td>
                        <td>
                            <div class="content-cell">
                                <div class="content-title">{{ Str::limit($post->title, 50) }}</div>
                            </div>
                        </td>
                        ...
                        <td>
                            @if($post->published_at)
                                <span class="date-cell">{{ $post->published_at->format('M d, Y') }}</span>
                            @else
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('blog-posts.show', $post) }}" class="btn-icon btn-icon-view" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                    </svg>
                                </a>
                                <a href="{{ route('blog-posts.edit', $post) }}" class="btn-icon btn-icon-edit" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                </a>
                                <form action="{{ route('blog-posts.destroy', $post) }}" method="POST" class="inline-form" data-confirm-delete="Are you sure you want to delete this blog post?" data-delete-title="Delete Blog Post">
                                    @csrf
                                    @method('DELETE')
    ...
    @if($blogPosts->hasPages())
        <div class="table-footer">
            <div class="table-info">
                Showing {{ number_format($blogPosts->firstItem()) }} to {{ number_format($blogPosts->lastItem()) }} of {{ number_format($blogPosts->total()) }} blog posts
            </div>
            <div class="pagination-wrapper">
                {{ $blogPosts->withQueryString()->links('vendor.pagination.custom') }}
            </div>
        </div>
    @endif
</div>
